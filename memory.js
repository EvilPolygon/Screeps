module.exports = {
    ...
    run: function(){
        ...
        if(Memory.Economical_Report.Ticks_for_next_report <= 0){
            ...
            Memory.Economical_Report.Ticks_for_next_report = 600;
        }
        ...
        for (let i in Memory.offenseINFO){
            ...
            if(Memory.offenseINFO[i].min_group == 0){Memory.offenseINFO[i].READY = true}
        ...
        //-------------------- CLAIMERS INFO ----------------------
        let cmem = Memory.claimINFO
        for (let i in cmem){
            if(Object.keys(cmem[i]) == 0){
                cmem[i] = {
                    targetRoom:false,
                    forClaim:false,
                    liveClaimer:false,
            ...
            let need_claimer = false;
            if(Game.rooms[cmem[i].targetRoom] != undefined){
                need_claimer = Game.rooms[cmem[i].targetRoom].controller.reservation != undefined ? Game.rooms[cmem[i].targetRoom].controller.reservation.ticksToEnd < 4000 : true;
            }
            else{
                need_claimer = true;
            }
            let claimer = _.filter(Game.creeps, c => c.memory.role == 'claimer' && c.memory.target == cmem[i].targetRoom);
            if(claimer[0]){
                need_claimer = need_claimer && claimer[0].ticksToLive < 70;
            }
            else{
                need_claimer = need_claimer && true;
            }
            need_claimer = need_claimer && claimer.length <= 1;
            need_claimer = need_claimer && _.find(Game.spawns, s => s.spawning && Game.creeps[s.spawning.name].memory.role == 'claimer' && Game.creeps[s.spawning.name].memory.target == cmem[i]) == undefined;  
            cmem[i].liveClaimer = !need_claimer;
        }
        //-----------------------------------------------------------
