        ...
        if(!creep.memory.work_with_link){
            if (creep.memory.isHarvesting){
                ...
                if(containers){
                    ...
                    let scrap = creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES,{filter: resource => resource.amount > 25 && resource.resourceType == 'energy'});
                ...
                let ruins = creep.pos.findClosestByRange(FIND_RUINS, { filter: t => t.store.getUsedCapacity('energy') > 0 });
                
                if(ruins){
                    if(creep.withdraw(ruins, 'energy') == -9){
                        creep.moveTo(ruins);
                    }
                }
