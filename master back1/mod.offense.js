module.exports = {
    ...
    run: function (creep) {
        ...
        if (creep.memory.role == 'marauder') {
            ...
            if (creep.memory.Path.length > 0) {
                ...
                if (creep.memory.take) {
                    ...
                    if (creep.store.getFreeCapacity() == creep.store.getCapacity() && creep.ticksToLive > 500) {
        ...
        else{//---------------------- НАЧАЛО КОДА ДЛЯ ОСТАЛЬНЫХ ЮОЕВЫХ ЕДИНИЦ -------------------------------------------
                ...
                if(creep.memory.tRoom != creep.room.name){
                    ...
                    switch(creep.memory.role){
                        ...
                        case 'avangard': {
                            ...
                            let InvCORE = creep.room.find(FIND_HOSTILE_STRUCTURES, {filter: C=> C.structureType = STRUCTURE_INVADER_CORE});console.log(JSON.stringify(InvCORE[0].pos));
                            
                            if(InvCORE.length>0){
                                if(creep.attack(InvCORE[0]) == -9){
