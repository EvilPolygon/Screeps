        ...
        if(creep.ticksToLive < 250){
            for( let i in Memory.claimINFO){
                if(Memory.claimINFO[i].targetRoom == creep.memory.target){
                    if(_.filter(Game.creeps, creep => creep.memory.role == 'claimer' && creep.memory.target == Memory.claimINFO[i].targetRoom).length == 1){
                        Memory.claimINFO[i].liveClaimer = false;
                    }
                }
            }
        }
