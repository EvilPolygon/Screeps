                    ...
                    if (!SPAWN[sp].spawning) {
                        ...
                        if (targetRooms.length > 0 && targetRooms[0] != null) {
                            for (let i in targetRooms) {
                                ...
                                if (!targetRooms[i].liveClaimer  && targetRooms[i].targetRoom && Game.map.findRoute(SPAWN[sp].room.name, targetRooms[i].targetRoom).length == 1 && SPAWN[sp].room.controller.level > 3) {
                                    ...
                                    case 'upgraders': {
                                        if (Game.rooms[SPAWN[0].room.name].memory.links != undefined) {
                                            ...
                                            for (let np = 1; np < cparts + 1; np++) {
                                                ...
                                                parts.push('work');
                                                parts.push('move');
                                                if (np > 1 && Object.keys(SPAWN[0].memory.Info.sources).length == 1) {
                                                    break;
                                                }
                                                if (np > 2) {
                                                    break;
