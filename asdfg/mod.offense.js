        ...
        if (creep.memory.role == 'marauder') {
            ...
            if (creep.memory.Path.length > 0) {
                if (creep.room.name != creep.memory.Path[0].room) {
                    creep.moveTo(creep.pos.findClosestByPath(creep.memory.Path[0].exit), { reusePath: 20, ignoreCreeps: true});
                }
                else {
                    ...
                    let target = creep.pos.findClosestByRange(FIND_STRUCTURES, {
                        filter: structure =>{
                            return (structure.structureType == 'storage' ||
                                ...
                                structure.structureType == 'container') && structure.store.getUsedCapacity() > 0;
                    }})
            ...
            let DANGER = creep.room.find(FIND_HOSTILE_CREEPS).length > 0;
                ...
                if(creep.memory.tRoom != creep.room.name && !DANGER){
                    ...
                    switch(creep.memory.role){
                        ...
                        case 'avangard': {
                            ...
                            let InvCORE = creep.room.find(FIND_HOSTILE_STRUCTURES, {filter: C=> C.structureType = STRUCTURE_INVADER_CORE});console.log(JSON.stringify(InvCORE[0].pos));console.log(JSON.stringify(InvCORE));
